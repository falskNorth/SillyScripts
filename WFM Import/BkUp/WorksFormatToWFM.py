# Read through sheet 1 of xlsx
# Find the Columns 'Qty', 'Unit', 'Part N°', 'Parts Description', 'Supplier', 'List Price (£)', 'Nett Total (£)', 'Date Order Issued to Procurement Dept.', 'Status'
# Format the 'Date Order Issued to Procurement Dept.' to DD/MM/YYYY
# Mark all empty rows to delete
        #check all rows with something in column  Parts Description that there is something in all other columns too if not mark for delete
            #Do not include all rows marked to delete in the CVS port
# Order the date in line with the headers for the CVS sheet ignoring the ones marked Delete
    # the Headers are "Job Number", "Cost Name", "Date (DD/MM/YYYY)", "Unit Cost", "Unit Price", "Quantity", "Phase", "Supplier", "Cost Code", "Type", "Billable", "Cost Note", "Estimated/Actual"
    # CVS Sheet name  = the Xlsx name:
# "Job Number" Leave Blank
# "Cost Name" = Parts Description
# "Date (DD/MM/YYYY)" = Date Order Issued to Procurement Dept.
